...
export default function ProjectsPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Load projects, clients, and tasks first
      ...
      try {
        ...
        console.log('⚠️ User.me() failed:', userError.message);
        // Set empty users array if authentication fails
        setUsers([]);
      }
    } catch (e) {
  ...
  const { activeProjects, archivedProjects } = useMemo(() => {
    ...
    console.log('🔍 Sample active project:', active[0]);
  ...
  const paginatedProjects = useMemo(() => {
    console.log('🔍 Starting pagination. Active projects:', activeProjects.length);
  ...
  const handleAddDefaultTasks = async (project) => {
    if (!confirm(`Add all default task templates to "${project.name}"?`)) {
      return;
    }
