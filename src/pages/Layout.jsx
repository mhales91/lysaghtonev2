...
const allNavigationItems = [
  ...
  { title: "TOE Builder", url: createPageUrl("TOEBuilder"), icon: FileText, roles: ["Staff", "Manager", "DeptLead", "Director", "Admin"] },
...
const adminNavigationItems = [
  ...
  { title: "AI Assistants", url: createPageUrl("AIAssistantManager"), icon: Bot },
...
const ProtectedLayout = ({ children, currentPageName }) => {
  console.log('ProtectedLayout is being rendered for:', currentPageName);
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        console.log('ProtectedLayout: Attempting to fetch user...');
        const user = await User.me();
        console.log('ProtectedLayout: User fetched successfully:', user?.email);
        ...
        if (!user.approval_status || user.approval_status === null || user.approval_status === undefined) {
          console.log('Auto-approving existing user without approval status');
          try {
            ...
            // Continue anyway - don't block existing users
          }
        }
        
        // Check if user is approved (after potential auto-approval)
        if (user.approval_status !== 'approved') {
          console.log('User not approved, showing pending approval message');
      ...
      <div className="min-h-screen flex w-full" style={{ backgroundColor: 'var(--lysaght-background)' }}>
        ...
        <main className="flex-1 flex flex-col overflow-hidden">
          ...
          >
            ...
            <h1 className="text-xl font-bold" style={{ color: 'var(--lysaght-text)' }}>
              Lysaght One
            </h1>
          </header>
...
export default function Layout({ children, currentPageName }) {
  ...
  console.log('Current Page Name in Layout:', currentPageName);
  console.log('Current URL:', window.location.href);
  console.log('Current Pathname:', window.location.pathname);
  
  const isPublicPage = currentPageName === 'TOESign';
  console.log('Is Public Page (TOESign)?:', isPublicPage);
  console.log('Comparison result:', currentPageName, '===', 'TOESign', '=', currentPageName === 'TOESign');

  if (isPublicPage) {
    console.log('✓ Rendering PUBLIC layout for TOE signing');
    return (
  ...
  console.log('✗ Rendering PROTECTED layout - user must authenticate');
