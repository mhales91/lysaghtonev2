// TOE PDF Generation Function
// Based on Base44 generateTOEPDF function, adapted for Supabase
...
function extractImageBytes(input) {
  ...
  if (m) {
    ...
    return { bytes: base64ToBytes(m[3]), type };
  }
  
  // Check for bare base64
  const maybeB64 = s.replace(/\s+/g, '');
  if (/^[A-Za-z0-9+/=]+$/.test(maybeB64)) {
    try {
      const head = atob(maybeB64.slice(0, 24));
      const isPNG = head.startsWith('\x89PNG\r\n\x1a\n');
      return { bytes: base64ToBytes(maybeB64), type: isPNG ? 'PNG' : 'JPEG' };
    } catch (e) {
      // ignore
    }
  }
...
export default async function generateTOEPDF(req) {
  try {
    ...
    // Note: This is a simplified version without PDF generation
    ...
    const pdfData = {
      ...
      fee_structure: fee_structure.map(item => ({
