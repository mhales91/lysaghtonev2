// TOE Operations Function
// Handles TOE-specific operations like creating projects from signed TOEs
...
export default async function toeOperations(req) {
  try {
    const { method } = req;
    ...
    if (method === 'POST' && action === 'create_project') {
      return await createProjectFromTOE(req);
    } else if (method === 'POST' && action === 'generate_share_link') {
      return await generateShareLink(req);
    } else if (method === 'GET' && action === 'get_share_data') {
      return await getShareData(req);
    } else {
...
async function createProjectFromTOE(req) {
  try {
    ...
    // Get the TOE data
    const { data: toe, error: toeError } = await supabase
      .from('toe')
      .select('*')
      .eq('id', toe_id)
      .single();
...
async function generateShareLink(req) {
  try {
    const payload = await req.json();
    const { toe_id } = payload;
...
async function getShareData(req) {
  try {
    const url = new URL(req.url);
