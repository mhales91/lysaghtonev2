// Signature Handling Function for Supabase Edge Functions
...
serve(async (req) => {
  ...
  try {
    ...
    if (method === 'POST') {
      return await createSignature(req, supabaseClient)
    } else if (method === 'PUT') {
      return await updateSignature(req, supabaseClient)
    } else if (method === 'GET') {
      return await getSignature(req, supabaseClient)
    } else {
...
async function createSignature(req: Request, supabaseClient: any) {
  try {
...
async function updateSignature(req: Request, supabaseClient: any) {
  try {
    const payload = await req.json()
...
async function getSignature(req: Request, supabaseClient: any) {
  try {
    ...
    let query = supabaseClient.from('toe_signature').select('*')
