// Fixed Base44 data import script
// Run this with: node import-base44-fixed.js
...
async function importBase44Data() {
  console.log('🚀 Starting fixed Base44 data import...');
  ...
  try {
    ...
    // Split into sections using a different approach
    const sections = exportData.split(/\n=== [A-Z]/);
    ...
    console.log('✅ Fixed Base44 data import completed!');
...
async function importCompanySettings(section) {
  ...
  const data = parseCSV(section);
  if (data.length === 0) {
    console.log('⚠️ No company settings data found');
    return;
  }
